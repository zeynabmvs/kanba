...
export const findTaskIndexById = (state, taskId) => {
  for (let targetBoardIndex = 0; targetBoardIndex < state.boards.length; targetBoardIndex++) {
    const board = state.boards[targetBoardIndex];
    for (let targetListIndex = 0; targetListIndex < board.lists.length; targetListIndex++) {
...
export const findSubtaskIndexes = (state, subtask, task) => {
  ...
  if (taskIndexes) {
    const subtaskIndex = task.subtasks.findIndex((item) => item.id === subtask.id);
...
export const getCurrentBoardIndex = (state) => {
  return state.boards.findIndex((item) => item.id === state.currentBoardId);
};
